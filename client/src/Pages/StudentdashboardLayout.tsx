...
const StudentdashboardLayout = () => {
  ...
  useEffect(() => {
    // Check if the userType exists in localStorage on load
  ...
  useEffect(() => {
    // Ensure redirection logic triggers only after state update
  ...
  }, [userType, navigate, location.pathname]); // Add location.pathname dependency to trigger navigation
  ...
  const navItems = [
    ...
    { name: "My Exams", icon: <FaBook />, path: "/student/my-exams" },
    { name: "Results", icon: <FaChartBar />, path: "/student/results" },
  ];

  const handleRoleChange = (role: string) => {
    setUserType(role);
    localStorage.setItem("userType", role); // Store userType in localStorage
              ...
              <div className="absolute top-full right-0 mt-2 bg-white border border-gray-300 shadow-md rounded-md z-10">
                {["Admin", "Student", "Logged in"].map((role) => (
                  <span
                    ...
                    onClick={() => handleRoleChange(role)} // Update role
                  >
                    {role}
                  </span>
                ))}
